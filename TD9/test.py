...
class FrodoKEM(object):
    ...
    def decode(self, K):
        ...
        k = [0 for i in range(self.B * self.mbar * self.nbar)]
        # 1. for i = 0; i < mbar; i += 1
        for i in range(self.mbar):
            # 2. for j = 0; j < nbar; j += 1
            for j in range(self.nbar):
                ...
                for l in range(self.B):
                    ...
                    k[(i * self.nbar + j) * self.B + l] = tmpbits[l]
    ...
    def pack(self, C):
        ...
        b = [0 for i in range(self.D * n1 * n2)]
        # 1. for i = 0; i < n1; i += 1
        for i in range(n1):
            # for j = 0; j < n2; j += 1
            for j in range(n2):
...
test_encode = test.encode(k)
print_mat(test_encode)

test_decode = test.decode(test_encode)
print(test_decode)
#print(k == test_decode)
###For the tests above rmb to change to array an not bit string

test_pack = test.pack(test_encode)
print(test_pack)

test_unpack = test.unpack(test_pack, 8, 8)
print_mat(test_unpack)
print(test.T_chi)

test.genAES128(0)
#print(test.sample(get_bytes()))
